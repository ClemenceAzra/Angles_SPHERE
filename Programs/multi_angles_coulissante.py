...
t_fnc_reconstr_approx_same_all=[]
d_norm_same_all=[]
...
for files in range(0,len(name_files)):
    ...
    for i in range(0,event_diapason.shape[1]):
       ...
       #Condition 3 for bins: delete the PMT with 0 bins
        ...
        # index_sig_max.append(imax_pmt) #index where sig max in each PMT
    ...
    #Verify the front t increase

    t_N=pd.DataFrame({'t':t_front-min(t_front),'N':N_photons,'d':np.sqrt(x_front**2+y_front**2)}).sort_values(by=['t'])


    # to show that it's the same with different d0
    # d_norm=t_N['d']-d0_from_center
    # a_t,b_t,c_t=np.polyfit(np.linspace(d_norm.min(),d_norm.max(),len(t_front)),list(t_N['t']-t_N['t'].min()),2)
    # t_fnc_reconstr_approx=a_t*np.linspace(d_norm.min(),d_norm.max(),len(t_front))**2+b_t*np.linspace(d_norm.min(),d_norm.max(),len(t_front))+c_t

    d_norm_same=t_N['d']-100
    xx_approx=np.linspace(d_norm_same.min(),d_norm_same.max(),len(t_front))
    a_t,b_t,c_t=np.polyfit(np.linspace(d_norm_same.min(),d_norm_same.max(),len(t_front)),list(t_N['t']-t_N['t'].min()),2)

    t_fnc_reconstr_approx_same=a_t*np.linspace(d_norm_same.min(),d_norm_same.max(),len(t_front))**2+b_t*np.linspace(d_norm_same.min(),d_norm_same.max(),len(t_front))+c_t
...
#  # ============== Minuit
    ...
    t_try.append(list(t_fnc_reconstr_approx)+[np.nan]*(109-len(t_fnc)))
    t_try_x.append(np.linspace(d_norm.min(),d_norm.max(),len(t_front)).tolist() +[np.nan]*(109-len(t_fnc)))
    
    t_fnc_reconstr_approx_same_all.append(t_fnc_reconstr_approx_same.tolist()+[np.nan]*(109-len(t_fnc)))
    d_norm_same_all.append(np.linspace(d_norm_same.min(),d_norm_same.max(),len(t_front)).tolist() +[np.nan]*(109-len(t_fnc)))
    ...
    nmbr_pmt.append(len(x_front))
    d0_real.append(np.sqrt(x0_real**2+y0_real**2))

    theta_total.append(theta*180/np.pi)

    x00.append(x0)
    y00.append(y0)
    
    x00_real.append(x0_real)
    y00_real.append(y0_real)
    
    a_t_increase.append(a_t)
    b_t_increase.append(b_t)
    c_t_increase.append(c_t)
    
    a_front.append(param_fix.values[2])
    b_front.append(param_fix.values[3])
    c_front.append(param_fix.values[4])

    
###==================================================================
###==================================================================

print(np.mean(delta_total))
#Analyse t increase

# params_t_increase=pd.DataFrame({'a':a_t_increase,'b':b_t_increase,'c':c_t_increase})

# a_mean,b_mean=params_t_increase['a'].mean(), params_t_increase['b'].mean()
# a_std,b_std=params_t_increase['a'].std(), params_t_increase['b'].std()

# curves_approx=pd.DataFrame({'dx_0':np.concatenate(d_norm_same_all),'t_y':np.concatenate(t_fnc_reconstr_approx_same_all)}).dropna()
# a_all,b_all,c_all=np.polyfit(list(curves_approx['dx_0']) , list(curves_approx['t_y']),2)

# x_approx_of_all_approx=d_norm_same_all
# approx_of_all_approx=a_all*np.array(x_approx_of_all_approx)**2+b_all*np.array(x_approx_of_all_approx)+c_all
